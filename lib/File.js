...
const settings = {
  ...
  get warehouse() {
    return path.resolve(`${this.location}/__fileSaver`);
  },
};
...
class File {
  ...
  async get(name, options = {}) {
    try {
      ...
      const data = await fs.readFile(key, { ...options, encoding: ENCODING, });
  ...
  async set(name, data, options = {}) {
    try {
      ...
      await fs.writeFile(key, JSON.stringify(data), { ...options, encoding: ENCODING, });
      return true;
    } catch (e) {
      console.log(e)
      return false;
